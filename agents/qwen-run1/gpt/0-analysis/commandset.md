...
| `qwen36 use-engine` | Selects the inference engine, either explicitly (`cpu`, `cuda`) or automatically (`--auto`). | The install hook invokes `qwen36 use-engine --auto --assume-yes`. The selected engine later drives `qwen36 show-engine`, which `apps/server.sh` parses to locate the engine name and required components. The command therefore appears to persist engine choice and engine-derived config into the CLI config store. |
| `qwen36 show-engine` | Prints the currently selected engine as YAML. | `apps/server.sh` parses `qwen36 show-engine | yq .components[]` and `qwen36 show-engine | yq .name`, so the command emits a YAML document containing at least `name` and `components`. The returned data likely corresponds to the selected `engines/*/engine.yaml` manifest plus resolved defaults. |
...
| `qwen36 set <key>=<value>` | Writes one config value. | The install hook uses `qwen36 set --package` to seed package defaults. User-facing examples use plain `qwen36 set http.port=8326`. The command appears to update the same config store that `get` resolves, possibly with multiple precedence layers. |
...
- `qwen36 status`
- `qwen36 unset`
- `qwen36 list-engines` or equivalent discovery command
