...
- Document exit code meanings for every leaf command.
- Distinguish configuration lookup miss from parser misuse.
- Document whether `use-engine --auto` can fail due to unsupported hardware versus missing permissions.
- Publish the stderr wording that scripts should not parse.
- Reserve distinct exit codes for usage error versus runtime failure, especially for `get`, `set`, and `use-engine`.