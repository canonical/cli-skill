...
This review was run directly on the local directory target `/project/tests/todo-07`. No active Pull Request context was detected.
...
| [MEDIUM-3](#medium-3-inconsistent-output-format-flags) | Inconsistent output format flags are used. The client `todo` query commands use `--output` while the daemon `todod status` uses `--format`. | [Parameters, Flags and Options](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#parameters-flags-and-options) | Standardize flag naming so all commands use `--format` consistently. |
...
### [MEDIUM-3] Inconsistent output format flags
**CLI Standard citation:** [Parameters, Flags and Options](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#parameters-flags-and-options) — *"All query commands must consistently support output format flags (--format or -f)"*
**Evidence:**
```go
// cmd/todo/main.go
...
```
The client CLI uses `--output` as the output format flag for `todo` commands, whereas the daemon CLI uses `--format` for the `todod status` command:
```go
// cmd/todod/main.go
	statusCmd.Flags().StringVar(&format, "format", "table", "Output format: table|json")
```
This violates consistency of option naming.
**Remediation:** Standardize the output format flags across all commands to `--format` or `-f`.
