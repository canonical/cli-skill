...
**Scope:** CLI standard compliance review of `/project/tests/todo-10`
...
| [MEDIUM-3](#medium-3-noun-used-instead-of-verb-for-primary-object-command) | The show command for todo is renamed to `info`, which uses a noun instead of a verb. | [Grammar + Vocabulary](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#commands-are-verbs) | Rename `info` to `show` for consistency. |
...
| [LOW-5](#low-5-secondary-object-detail-command-uses-show-foobar-instead-of-foobar) | The sink detail command uses `show-sink <sink-id>` instead of shorthand `sink <sink-id>`. | [Grammar + Vocabulary](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#showing-state-shorthand) | Rename `show-sink` to `sink`. |
...
### [MEDIUM-3] Noun used instead of verb for primary object command
...
```go
	showCmd := &cobra.Command{
		...
		Short: "Show a todo",
```
The command `info` uses a noun instead of a verb to display a specific todo. Commands acting on the primary object must be verbs (such as `show`).
**Remediation:** Rename the command from `info <todo-id>` to `show <todo-id>`.
...
### [LOW-5] Secondary object detail command uses show-foobar instead of foobar
**CLI Standard citation:** [Grammar + Vocabulary](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#showing-state-shorthand) — *"For specific secondary objects, use foobar-status over show-foobar-status (e.g. snapcraft release-status), and foobar over show-foobar."*
**Evidence:**
```go
	sinkCmd := &cobra.Command{
		...
		Short: "Show a sink",
```
The command to show details of a secondary object `sink` uses `show-sink <sink-id>` instead of the shorthand `sink <sink-id>`.
**Remediation:** Rename the `show-sink` command to `sink`.
...
- **Verbs represent actions**: All subcommands that act on primary or secondary objects (except `info` and `show-sink`) are named using standard verbs (e.g., `list`, `create`, `update`, `close`, `reopen`, `reject`).
