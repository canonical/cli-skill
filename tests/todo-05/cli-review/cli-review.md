...
**Scope:** CLI standard compliance review of `/project/tests/todo-05/cmd/todo/main.go` and `/project/tests/todo-05/cmd/todod/main.go`

## Summary

| **Severity** | **Count** | **Guideline Categories** |
| --- | --- | --- |
| High | 1 | Positional Parameters |
...
* **Decreased Positional Argument Clarity:** The positional argument placeholder for the `todo show` command was modified from `<todo-id>` to `<id>`. This violates positional argument clarity and creates inconsistency with other todo commands that use `<todo-id>` (such as `update <todo-id>`, `close <todo-id>`, etc.).
...
### [HIGH-1] `todo show` positional argument naming violates clarity rules
...
```go
	showCmd := &cobra.Command{
		...
		Short: "Show a todo",
```
The command `show` represents retrieving details for a single `todo` primary object. However, its positional argument placeholder is specified as `<id>` instead of `<todo-id>`. This creates inconsistency and ambiguity across the CLI, as other commands operating on individual todos consistently use the placeholder `<todo-id>` (e.g., `update <todo-id>`, `close <todo-id>`, `reopen <todo-id>`, `reject <todo-id>`).

**Remediation:** In `cmd/todo/main.go`, change the command `Use` string to use `<todo-id>` instead of `<id>`:
```go
	showCmd := &cobra.Command{
		Use:   "show <todo-id>",
```
