...
**Scope:** CLI standard compliance review of `/project/tests/todo-13`
...
| --- | --- | --- |
| High | 0 | None |
| Medium | 3 | Grammar + Vocabulary, Action Verb Alignment |
...
| [MEDIUM-3](#medium-3-inconsistent-verb-form-mark-closed-instead-of-standard-verb-close) | The action command for closing a todo is named `mark-closed` instead of `close`, which is inconsistent with other state-changing commands (`reopen` and `reject`). | [Grammar + Vocabulary](https://github.com/canonical/cli-skill/blob/main/cli-skill/references/cli-standard.md#rule-commands-are-verbs) | Rename `mark-closed` to `close` to align with standard verb usage and consistency. |
...
### [MEDIUM-3] Inconsistent verb form `mark-closed` instead of standard verb `close`
...
```
The CLI uses the verb-adjective phrase `mark-closed` to change a todo's state, while using single-verb forms `reopen` and `reject` for other state changes. This introduces inconsistency and verb-form misalignment on the primary object actions.
**Remediation:** Rename the `mark-closed` command to the standard, consistent single-verb form `close`.
...
- **Verbs represent actions**: All subcommands that act on primary or secondary objects are named using standard verbs (e.g., `list`, `show`, `create`, `update`, `reopen`, `reject`).
