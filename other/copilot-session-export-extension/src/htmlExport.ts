import { SessionData } from './parser';
...
function generateSubAgentSvg(session: SessionData): string {
  ...
  const groups: typeof agents[] = [];
  let currentGroup: typeof agents = [agents[0]];
...
export function generateHtml(sessions: SessionData[]): string {
