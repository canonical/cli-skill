...
def find_cli_doc(root: pathlib.Path) -> pathlib.Path:
    kb_dir = root / "knowledge_base"
    if not kb_dir.exists():
        fail(
            "Missing knowledge_base/CLI.md. This workflow only runs for repositories that declare CLI scope metadata. "
            "Add knowledge_base/CLI.md. For repositories with a CLI, include a YAML entry like "
            "'paths: [cmd/, internal/cli/, docs/cli/]' listing all repo paths that affect CLI behavior, help, or command UX. "
...
def main() -> None:
    ...
    with output_path.open("a", encoding="utf-8") as file_handle:
        if cli_present is False:
            file_handle.write("proceed=false\n")
    ...
    matched = [item for item in changed if any(matches(spec, item) for spec in paths)]

    with output_path.open("a", encoding="utf-8") as file_handle:
        if matched:
            file_handle.write("proceed=true\n")
        else:
            file_handle.write("proceed=false\n")
